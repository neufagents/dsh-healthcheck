...
export function analyzeLibrary(sessions, topN = 5) {
    ...
    for (const s of sessions) {
        ...
        if (sz >= BIG_SIZE_DANGER)
            warnings.push(`[高危] 会话 ${shortId(s.id)} 体积 ${formatBytes(sz)}(≥30MB,OOM 风险高)`);
        else if (sz >= BIG_SIZE_WARN)
            warnings.push(`[关注] 会话 ${shortId(s.id)} 体积 ${formatBytes(sz)}(≥10MB)`);
        const ev = s.eventCount ?? 0;
        if (ev >= BIG_EVENTS_DANGER)
            warnings.push(`[高危] 会话 ${shortId(s.id)} 事件数 ${ev}(≥8000,上下文膨胀严重)`);
        else if (ev >= BIG_EVENTS_WARN)
            warnings.push(`[关注] 会话 ${shortId(s.id)} 事件数 ${ev}(≥3000)`);
    }
    return { total, totalBytes, totalEvents, largest, busiest, warnings };
}
export function analyzeLog(events) {
    const typeCounts = {};
    let toolCalls = 0;
    let toolResults = 0;
    let lastTurnEvent;
    ...
    const hints = [];
    if (events.length === 0)
        hints.push('空日志(会话已创建但未写入事件)。');
    if (!balanced)
        hints.push('检测到未闭合的轮次尾部(疑似崩溃遗留):dsh 在下次 resume 时会自动补全,无需手动修复。');
    if (unmatched > 0)
        hints.push(`${unmatched} 个工具调用缺少结果记录(崩溃或中止的正常特征;resume 时会被补上错误结果)。`);
    return {
...
// ---------- 报告生成 ----------
export function formatLibraryReport(stats) {
    const lines = [];
...
export function formatLogReport(id, snap, a) {
    const lines = [];
    ...
    if (a.lastTurnEvent)
        lines.push(`- 最后一个轮次事件:\`${a.lastTurnEvent}\``);
